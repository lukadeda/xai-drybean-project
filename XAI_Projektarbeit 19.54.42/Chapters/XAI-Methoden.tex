...
\section{Modell- und Methodenauswahl}
...
\section{Lokale Erklärung einer korrekten Vorhersage}

Neben der globalen Sicht wurde zuerst eine korrekt klassifizierte Testbeobachtung betrachtet. Das Modell sagt für diese Beobachtung \texttt{DERMASON} vorher, und die tatsächliche Klasse ist ebenfalls \texttt{DERMASON}. Die lokale SHAP-Erklärung bezieht sich auf genau diese vorhergesagte Klasse.
...
\section{Lokale Erklärung einer Fehlklassifikation}

Als zweiter lokaler Fall wurde eine Fehlklassifikation aus einem wiederkehrenden Fehlermuster gewählt. Die tatsächliche Klasse ist \texttt{DERMASON}, der Random Forest sagt aber \texttt{SIRA} vorher. Dieser Fall ist für die Bewertung aussagekräftiger als eine weitere richtige Vorhersage, weil \texttt{DERMASON} und \texttt{SIRA} bereits in den Konfusionsmatrizen häufig verwechselt werden.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{Graphics/SHAP_Local_Misclassified_RandomForest.pdf}
  \caption{Lokale SHAP-Erklärung einer DERMASON-Instanz, die als SIRA klassifiziert wurde}
  \label{fig:SHAP_Local_Misclassified_RandomForest}
\end{figure}

Abbildung~\ref{fig:SHAP_Local_Misclassified_RandomForest} zeigt die SHAP-Beiträge für die vorhergesagte Klasse \texttt{SIRA}. Besonders stark in Richtung \texttt{SIRA} wirken ShapeFactor1, Roundness, Area und Compactness. ShapeFactor2 trägt ebenfalls positiv bei. Die Beiträge von Eccentricity und AspectRatio sind kleiner. ShapeFactor4 wirkt in diesem Fall leicht gegen die Vorhersage \texttt{SIRA}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{Graphics/LIME_Local_Misclassified_RandomForest.pdf}
  \caption{Lokale LIME-Erklärung derselben DERMASON-Instanz, die als SIRA klassifiziert wurde}
  \label{fig:LIME_Local_Misclassified_RandomForest}
\end{figure}

Die LIME-Erklärung in Abbildung~\ref{fig:LIME_Local_Misclassified_RandomForest} zeigt ein ähnliches Muster. Die stärksten positiven Gewichte für \texttt{SIRA} liegen bei einer mittleren \texttt{Area}, bei \texttt{ShapeFactor1}, \texttt{Compactness} und \texttt{Roundness}. \texttt{ShapeFactor4} erhält ein kleines negatives Gewicht. Die beiden lokalen Methoden erklären die Fehlklassifikation also nicht als Ausreißer, sondern als Fall, dessen Größen- und Formwerte aus Sicht des Modells eher zur Klasse \texttt{SIRA} passen.

Dieser zweite lokale Fall macht die Grenze des Modells deutlicher. Die Erklärung ist fachlich plausibel, aber sie zeigt zugleich, warum die reine Modellleistung nicht ausreicht. Wenn zwei Sorten morphologisch nah beieinanderliegen, können dieselben Merkmalsgruppen die richtige Trennung unterstützen und in Grenzfällen zu einer Verwechslung beitragen.

\section{Vergleich der XAI-Methoden}

Die drei Methoden beantworten unterschiedliche Fragen. Permutation Feature Importance zeigt, welche Merkmale für die Testleistung des Random Forest wichtig sind. SHAP zeigt, wie stark Merkmale zu Vorhersagen beitragen, und kann von lokalen Erklärungen zu einer globalen Rangfolge verdichtet werden. LIME erklärt einzelne Vorhersagen über ein lokales Ersatzmodell. Durch die zwei lokalen Fälle wird sichtbar, wie sich die Methoden bei einer richtigen Vorhersage und bei einer typischen Fehlklassifikation verhalten.
...
Für die Forschungsfrage bedeutet das: Die Modelle liefern nicht nur hohe Klassifikationswerte, sondern ihre Entscheidungen lassen sich mit den verwendeten XAI-Methoden inhaltlich nachvollziehen. Die Aussage bleibt aber begrenzt. Stark korrelierte Merkmale erschweren die Interpretation, und lokale Erklärungen zeigen ausgewählte Einzelfälle. Für die weitere Diskussion ist deshalb entscheidend, die XAI-Ergebnisse zusammen mit der EDA, den Konfusionsmatrizen und den bekannten Fehlermustern zwischen \texttt{DERMASON}/\texttt{SIRA} sowie \texttt{BARBUNYA}/\texttt{CALI} zu lesen.
